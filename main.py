...
def count_grains(path, output_dir):
    ...
    for i, contour in enumerate(contours):
        if len(contour) >= 5:  # Minimum points needed for ellipse fitting
            ...
            if w <= 0 or h <= 0:
                print(f"Skipping contour {i} with invalid bounding box: ({x}, {y}, {w}, {h})")
                continue
            ...
            try:
                ...
                if minor_axis < 0 or major_axis < 0:
                    print(f"Skipping invalid ellipse for contour {i}: major={major_axis}, minor={minor_axis}")
                    continue
                ...
                print(f"OpenCV error for contour {i}: {e}")
                continue
